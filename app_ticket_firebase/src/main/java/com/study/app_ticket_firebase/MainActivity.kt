...
class MainActivity : AppCompatActivity() {
    ...
    fun buyTicket(view: View) {
        // 檢查 UI 欄位資料
        if (et_all_ticket.text.toString().length == 0) {
            return
        }
        ...
        // 進行購票
        val allTickets = et_all_ticket.text.toString().toInt()
        ...
        try {
            ...
            val ticket = TicketService().submit(userName, allTickets, roundTrip, ticketsStock)
            ...
            // 購買成功訊息
            tv_waring.text = resources.getString(R.string.waring_txt)
        } catch (e: Exception) {
            tv_waring.text = e.message
        }
